 ...
 * shape. <br>
 * <h3>Determining the control size</h3> If when creating the button you specify
 * a particular width and height then any images that are not the same size will
 * be scaled to fit without regard to the original size or aspect ratio. <br>
 * 
 * If when creating the button you do not specify the width and height then it
 * will use the width and height of the 'off-button' image and assume that all
 * the other images are the same size. <br>
 * 
 * <h3>The images</h3> The image button needs 1 to 3 image files to represent
 * the button states <br>
 ...
 * provide two then the second image is used for both OVER and DOWN states. <br>
 * <br>
 * 
 * If you don't provide a mask file then the button 'hotspot' is represented by
 * any non-transparent pixels in the OFF image. If you do provide a mask file
 * then the hotspot is defined by any black pixels in the mask image. <br>
 * <br>
 * 
 * 
...
public class GImageButton extends GAbstractControl {
	...
	/**
	 * The control size will be set to the size of the image file used for the
	 * button OFF state. <br>
	 ...
	 * @param fnames    an array of up to 3 image filenames to represent the
	 *                  off/over/down state of the button.
	 */
	...
	public GImageButton(PApplet theApplet, float p0, float p1, String[] fnames, String fnameMask) {
		...
		if (mask != null) { // if we have a mask use it for the hot spot
			...
			hotspots = new HotSpot[] { new HSalpha(1, 0, 0, bimage[0], PApplet.CORNER) };
		}
		// ========================================================================

		z = Z_SLIPPY;
		// Now register control with applet
		createEventHandler(G4P.sketchWindow, "handleButtonEvents", new Class<?>[] { GImageButton.class, GEvent.class },
	...
	/**
	 * 
	 * When a mouse button is clicked on a GImageButton it generates the
	 ...
	 * </pre>
	 * 
	 * is the GImageButton identifier (variable name) <br>
	 * <br>
	 * 
	 */
	 ...
	 * CLICKED events are generated (default behaviour).
	 * 
