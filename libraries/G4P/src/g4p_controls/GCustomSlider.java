 ...
 * with user provided graphics. The library provides a number of skins ready for
 * use. You specify the skin to use when the slider is created and if the
 * library is unable to load the skin it will print a warning and load the
 * default skin instead.
 * </p>
 * <p>
 ...
 * </ul>
 * <p>
 * A skin requires 5 image files for different parts of the slider which must be
 * stored in their own folder (the folder name is also used as the skin name)
 * and this folder should be place inside the sketch's data folder.
 * </p>
 * <p>
 ...
 * </ul>
 * <p>
 * Where ??? is the image type file extension. The image type can be any that
 * Processing can handle, the most common types will be png, jpg or gif but tga
 * is also permitted
 * </p>
 * 
 * <p>
 * There are very few restrictions about the images you use but when designing
 * the images you should consider the following facts:
 * </p>
 * <ul>
 * <li>the slider will be created to fit the control size (specified in the
 * constructor)</li>
 * <li>the horizontal space allocated for the end-caps will be the same for each
 * end (uses the width or the larger end cap image)</li>
 * <li>the track width will be the height of the centre image</li>
 * <li>the centre image will be tiled along the track length</li>
 * <li>the track will be placed in the horizontal and vertical centre of the
 * control.</li>
 * <li>the end cap images will be placed in the vertical centre of the control
 * and butted against the track.</li>
 * </ul>
 * 
 * 
 * @author Peter Lager
...
public class GCustomSlider extends GLinearTrackControl {
	 ...
	 * @param p2        x position or width based on control mode
	 * @param p3        y position or height based on control mode
	 * @param skin      the name of the skin (this is also the name of the folder
	 *                  holding the images)
	 */
	public GCustomSlider(PApplet theApplet, float p0, float p1, float p2, float p3, String skin) {
		...
		hotspots = new HotSpot[] {
				new HSalpha(THUMB_SPOT, width / 2 + (parametricPos - 0.5f) * trackLength, height / 2, thumb,
						PApplet.CENTER), // thumb
		...
		createEventHandler(G4P.sketchWindow, "handleSliderEvents", new Class<?>[] { GValueControl.class, GEvent.class },
	...
	private void loadSkin(String style) {
		...
		if (!style.equalsIgnoreCase(style_used))
			System.out.println(
					"Unable to load the skin " + style + " using default '" + DEFAULT_SLIDER_STYLE + "' style instead");
	}

	/**
	 * If no user defined skin has been specified then load a style from the G4P
	 * library. If the style does not exist it will use the default style.
	 * 
	 * @param style
	 */
	...
	private boolean loadSkin_AppletInBrowser(String style) {
		...
		boolean found = !(leftEnd == null || rightEnd == null || thumb == null || thumb_mouseover == null
				|| centre == null);
