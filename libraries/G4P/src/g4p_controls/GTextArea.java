 ...
 * 
 * This control allows the user to enter and edit multiple lines of text. The
 * control also allows default text, horizontal and vertical scrollbars. <br>
 * 
 * Enables user to enter text at runtime. Text can be selected using the mouse
 * or keyboard shortcuts and then copied or cut to the clipboard. Text can also
 * be pasted in. <br>
 * 
 * There are some methods to add and clear text attributes to all or some of the
 * text in the control. If a method is expecting a line number, you should
 * specify the actual line number for the entire text (lines numbers start at
 * 0). It is not the line number in the visible display because this can change
 * if the text has been scrolled vertically. <br>
 * 
 * 
...
public class GTextArea extends GEditableTextControl {
	...
	/**
	 * Create a text area without scrollbars and a text wrap width to fit the
	 * control.
	 * 
	...
	/**
	 * Create a text field with the given scrollbar policy and a text wrap width to
	 * fit the control. <br>
	 ...
	 * </ul>
	 * e.g. SCROLLBARS_BOTH | SCROLLBARS_AUTOHIDE <br>
	 * 
	 * @param theApplet the main sketch or GWindow control for this control
	 * @param p0        x position based on control mode
	 * @param p1        y position based on control mode
	...
	/**
	 * Create a text field with the given scrollbar policy with a user specified
	 * text wrap length <br>
	 * 
	 * @param theApplet the main sketch or GWindow control for this control
	 * @param p0        x position based on control mode
	...
	public GTextArea(PApplet theApplet, float p0, float p1, float p2, float p3, int sbPolicy, int wrapWidth) {
		...
		if ((scrollbarPolicy & SCROLLBAR_HORIZONTAL) != 0) {
			...
			addControlImpl(hsb, tx, ty + th + 2, 0);
		...
		if ((scrollbarPolicy & SCROLLBAR_VERTICAL) != 0) {
			...
			addControlImpl(vsb, tx + tw + 18, ty, PI / 2);
	...
	/**
	 * Set the text to be used. The wrap width is determined by the current text
	 * wrapwidth or if there is no text then the text width of the control.
	 * 
	...
	/**
	 * Adds the text attribute to a range of characters on a particular display
	 * line. If charEnd is past the EOL then the attribute will be applied to the
	 * end-of-line.
	 * 
	...
	/**
	 * Clears all text attribute from a range of characters on a particular display
	 * line. If charEnd is past the EOL then the attributes will be cleared to the
	 * end-of-line.
	 * 
	 * @param lineNo    the display line number (starts at 0)
	 * @param charStart the position of the first character to apply the attribute
	...
	private void displayCaretPos(String title) {
		if (endTLHI != null && endTLHI.tli != null && endTLHI.thi != null) {
			System.out.println(
					title + "  :: Carat on line " + endTLHI.tli.lineNo + "   at char " + endTLHI.thi.getCharIndex());
		} else {
		...
		if (bufferInvalid) {
			...
			for (TextLayoutInfo lineInfo : lines) {
				...
				if (!usePromptText && hasSelection() && lineInfo.compareTo(startSelTLHI.tli) >= 0
					...
					ee = (lineInfo.compareTo(endSelTLHI.tli) == 0) ? endSelTLHI.thi.getInsertionIndex()
							: lineInfo.nbrChars - 1;
