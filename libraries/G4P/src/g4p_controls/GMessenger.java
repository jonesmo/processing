...
class GMessenger implements GConstants, GConstantsInternal {
	 ...
	 * 
	 * info[0] event generator class info[1] event handling method name info[2] the
	 * exception thrown
	 * 
	 */
	 ...
	 * 
	 * info[0] event generator class info[1] event handling method name info[2] the
	 * parameter class names info[3] the parameter names (identifiers)
	 * 
	 */
	...
	/**
	 * An attempt was made to add a G4P control that does not have a visual
	 * appearance or one that does not respond to mouse / keyboard events.
	 * 
	...
	private static void unmatchedPApplet(Object[] info) {
		String groupClassName = info[0].getClass().getSimpleName();
		String className = info[1].getClass().getSimpleName();
		System.out.println("The " + className + " object cannot be added to this control group (" + groupClassName
				+ ") because they are for different windows.\n");
	}
}
