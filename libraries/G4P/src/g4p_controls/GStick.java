 ...
 * intuitive control in game scenarios where you might use the keyboard e.g.
 * WASD keys for movement. <br>
 * 
 * The joystick has two modes - in the default mode the joystick just responds
 * to movement in 4 directions (left,right, up and down) the second mode allows
 * for diagonals so recognises 8 directions. <br>
 * 
 * As in a real joystick you have a dead zone near the centre which does not
 * generate signals, thus avoiding jitter. This area is shown graphically. <br>
 * 
 * The direction of the joystick is represented by an integer in the range 0-7
 * and -1 when in the dead zone. <br>
 * 
 * <pre>
 *     5   6   7
 *      \  |  /
 ...
 * 
 * <br>
 * As well as the direction there are two useful methods to decode these into X
 * and Y directions -
 * 
 * <pre>
 * getStickX()
...
public class GStick extends GAbstractControl {
	...
	public GStick(PApplet theApplet, float p0, float p1, float p2, float p3) {
		...
		hotspots = new HotSpot[] { new HScircle(1, width / 2, height / 2, gripRad) };
		z = Z_SLIPPY;

		// Now register control with applet
		createEventHandler(G4P.sketchWindow, "handleStickEvents", new Class<?>[] { GStick.class, GEvent.class },
	 ...
	 * If the mode parameter should be either G4P.X4 or G4P.X8 any other value will
	 * be silently ignored
	 * 
