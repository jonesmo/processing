 ...
 * <br>
 * A toggle control can be in one of 2 states <b>selected</b> or <b>not
 * selected</b> and is the base class for the GOption and GCheckbox controls.
 * 
 * @author Peter Lager
 *
 */
public abstract class GToggleControl extends GTextIconBase {
	...
	public GToggleControl(PApplet theApplet, float p0, float p1, float p2, float p3) {
		...
		opaque = false;
		hotspots = new HotSpot[] { new HSrect(1, 0, 0, width, height) // control surface
		};
	}
	 ...
	 * 
	 * This is the only method that allows you to use an animated icon. Create and
	 * configure the icon before adding to this control. You can use the getIcon
	 * 
	 * You can pass <b>null</b> if you don't want to change a particular
	 * position/alignment. <br>
	 * 
	...
	public void setIcon(GIcon g_icon, GAlign pos, GAlign horz, GAlign vert) {
		...
		if (icon != null && icon instanceof GAnimIcon) {
	...
	/**
	 * This method is called if this control is added to a toggle group. A toggle
	 * group enforces single option selection from the group. Override this with an
	 * empty method to allow each toggle control to be independent of others.
	 * 
	 * @param tg
	 */
...
//		GAnimIcon ai = (GAnimIcon) icon.me();
		if (icon != null && icon instanceof GAnimIcon) {
			GAnimIcon ai = (GAnimIcon) icon;
			if (selected)
				ai.animate("SELECT");
