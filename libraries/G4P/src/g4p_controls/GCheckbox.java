...
public class GCheckbox extends GToggleControl {
	...
	public GCheckbox(PApplet theApplet, float p0, float p1, float p2, float p3, String text) {
		...
		setText(text);
		setIcon("tick.png", 2, GAlign.CENTER, GAlign.MIDDLE);
		// Setting the icon will resize the text and icon zones, so no need to do it
		// again.

		z = Z_SLIPPY;
		// Now register control with applet
		createEventHandler(G4P.sketchWindow, "handleToggleControlEvents",
				new Class<?>[] { GToggleControl.class, GEvent.class }, new String[] { "checkbox", "event" });
	...
	/**
	 * This enforces independent action because this control cannot be added to a
	 * toggle group
	 */
	@Override
