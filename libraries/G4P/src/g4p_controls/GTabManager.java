...
/**
 * Allows TABBING between text controls. A tab manager allows the user to use
 * the TAB key to move from one text control (GTextField or GTextArea) to the
 * another. This is useful when creating a 'form' made from several text
 * controls. <br>
 * The tab order is decided by the order the text controls are added to the tab
 * manager. The TAB key move the focus forwards and SHIFT+TAB moves it
 * backwards. Note that tabbing is not cyclic so pressing TAB on the last
...
public class GTabManager {
	 ...
	 * @return true if any or all of the controls were added and false if none were
	 *         added.
	 */
	...
	/**
	 * Remove a control from the tab manager. This does not affect the tab order of
