...
public class GDropList extends GTextBase {
	 ...
	 * @param p2        x position or width based on control mode
	 * @param p3        y position or height based on control mode
	 * @param listSize  the maximum number of element to appear in the drop down
	 *                  list
	 */
	public GDropList(PApplet theApplet, float p0, float p1, float p2, float p3, int listSize) {
		this(theApplet, p0, p1, p2, p3, listSize, BUTTON_WIDTH);
	}

	/**
	 * Create a drop down list component with a specified list size.<br>
	 * 
	 * The width of the show-list button is constrained between 10pixels and half
	 * the control width.<br>
	 * 
	 * IMPORTANT <br>
	 * After creating the control use setItems to initialise the list. <br>
	 * 
	 ...
	 * @param p2          x position or width based on control mode
	 * @param p3          y position or height based on control mode
	 * @param listSize    the maximum number of elements to appear in the drop down
	 *                    list
	...
	public GDropList(PApplet theApplet, float p0, float p1, float p2, float p3, int listSize, float buttonWidth) {
		...
		addControlImpl(vsb, width, itemHeight + 1, PI / 2);
		addControlImpl(showList, width - buttonWidth, 0, 0);

		G4P.popStyle();

		hotspots = new HotSpot[] {
				new HSrect(LIST_SURFACE, 0, itemHeight + 1, width - buttonWidth - 1, height - itemHeight - 1), // text
																												// list
																												// area
				new HSrect(CLOSED_SURFACE, 0, 0, width - buttonWidth, itemHeight) // selected text display area
		};

		createEventHandler(G4P.sketchWindow, "handleDropListEvents", new Class<?>[] { GDropList.class, GEvent.class },
	...
	/**
	 * Use this to set or change the items to appear in the list. If you enter an
	 * invalid selection index then it is forced into the valid range. <br>
	 * Null and empty values in the array will be ignored. <br>
	 * If the array is null then or empty then then no changes are made. <br>
	 * 
	 * @param array    the text to appear in the dropdown list elements
	 ...
	 * 
	 * @param list     the text to appear in the dropdown list elements
	 * @param selected the index of the selected element
	 */
	...
	/**
	 * Implements changes instigated by other metods i.e. add, insert and remove.
	 * 
	 * @param selected
	 */
	 ...
	 * If idx is not a valid position in the list then the list is unchanged. If idx
	 * points to the selected item or an item below it then the selected index value
	 * is reduced by 1 but the selected text remains the same. <br>
	 * If idx points to an item above the selected item then the selected item is
	 * unchanged. <br>
	 ...
	 * If idx points to the selected item or an item below it then the selected
	 * index value is incremented by 1 but the selected text remains the same. <br>
	 * If idx points to an item above the selected item then the selected item is
	 * unchanged. <br>
	 * No event is fired even if the selected index changes. <br>
	 * 
	...
	/**
	 * Determines if a particular pixel position is over this control taking into
	 * account whether it is collapsed or not.
	 * 
	...
	/**
	 * For most components there is nothing to do when they lose focus. Override
	 * this method in classes that need to do something when they loose focus eg
	 * TextField
	 */
	...
	/*
	 * This method should <b>not</b> be called by the user. It is for internal
	 * library use only.
	 */
