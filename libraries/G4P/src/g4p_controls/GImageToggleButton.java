...
/**
 * Buttons created from this class have 2 or more toggle states. If the number
 * of states is N then the button's value will be in the range 0 to N-1. Most
 * toggle buttons will have just two states and these have values 0 and 1. <br>
 * Clicking on the button advances the state by one, restarting at zero after
 * the last state. <br>
 * Each state must have its own 'picture' and the user must supply these as a
 * tiled image where the pictures are tiled in 1D or 2D arrangement without
 * 'empty space' around the tiles. <br>
 * If for any reason the library is unable to use the specified graphics then it
 * will provide a default two state toggle switch. <br>
 * It is also possible to provide an over-button image set for when the mouse
 * moves over the button - this is optional. <br>
 * The button control will always be resized to suit the state picture size
 * (tile size). <br>
 * The mouse is considered to be over the button it its position is over an
 * opaque pixel in the state picture. Since transparent pixels are not included
 * then the button shape can be different for each state. <br>
 * 
 * 
 * 
 ...
 * 
 * By default the button only fires the CLICKED event which is typical of most
 * GUIs. G4P supports two other events PRESSED and RELEASED which can be enabled
 * using
 * 
 * <pre>
 ...
 * button face. When the mouse button is released one of two events will be
 * generated, the RELEASED event if the mouse has moved since the PRESSED event
 * or CLICKED event if it has not moved. If you use this feature remember to
 * test the event type in the event-handler.<br>
 * 
 ...
 * </pre>
 * 
 * do not try this with the RELEASED or PRESSED event as it will lead to
...
public class GImageToggleButton extends GAbstractControl {
	 ...
	 * @param p1          vertical position of the control
	 * @param offPicture  the filename of bitmap containing toggle state pictures
	 * @param overPicture the filename of bitmap containing mouse-over button toggle
	 *                    state pictures
	 * @param nbrCols     number of tiles horizontally
	 */
	public GImageToggleButton(PApplet theApplet, float p0, float p1, String offPicture, String overPicture,
			int nbrCols) {
	...
	public GImageToggleButton(PApplet theApplet, float p0, float p1, String offPicture, String overPicture, int nbrCols,
		...
		PImage temp = null;
		if (nbrCols < 1 || nbrRows < 1 || offPicture == null
				|| null == (temp = ImageManager.loadImage(winApp, offPicture))) {
		...
		// ========================================================================
		// Setup the hotspots
		hotspots = new HotSpot[] { new HSalpha(1, 0, 0, offImage[stateValue], PApplet.CORNER) };
		...
		createEventHandler(G4P.sketchWindow, "handleToggleButtonEvents",
				new Class<?>[] { GImageToggleButton.class, GEvent.class }, new String[] { "button", "event" });
	...
	/**
	 * 
	 * When a mouse button is clicked on a GImageToggleButton it generates the
	 * GEvent.CLICKED event. If you also want the button to generate GEvent.PRESSED
	 * and GEvent.RELEASED events then you need the following statement.<br>
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * 
	 * is the GImageToggleButton identifier (variable name) <br>
	 * <br>
	 * 
	 */
	 ...
	 * If the parameter is not a valid toggle state value then it is ignored and the
	 * button's state value is unchanged.
	 * 
	...
	/**
	 * If the parameter is true all 3 event types are generated, if false only
	 * CLICKED events are generated (default behaviour). <br>
	 * For this toggle control I can't see the need for anything but CLICKED events
	 * 
	 * @param all true if we want all events fired
	 */
