 ...
 * G4P supports 16 colour schemes and each scheme has a <b>palette</b> of 16
 * colours. <br>
 * 
 * When G4P is used it loads an image file with all the colors used by the
 * various colour schemes. <br>
 * First it will search for a file containing a user defined scheme
 * (user_gui_palette.png) and if it can't find it, will use the library default
 * scheme (default_gui_palette.png).
 * 
 * @author Peter Lager
 *
 */
public class GCScheme implements GConstants, PConstants {
	...
	/**
	 * Set the color scheme to one of the preset schemes BLUE / GREEN / RED / PURPLE
	 * / YELLOW / CYAN / BROWN or if you have created your own schemes following the
	 * instructions at gui4processing.lagers.org.uk/colorscheme.html then you can
	 * enter the appropriate numeric value of the scheme.
	 * 
	...
	/**
	 * Save the current colour schemes as an image in the sketch's data folder. The
	 * file will be called
	 * 
	 * <pre>
...
//		System.out.println(filename);
	...
	/**
	 * Called every time we create a control. The palettes will be made when the
	 * first control is created.
	 * 
	...
	/*
	 * The following methods are ONLY called by GUI Builder do not change their
	 * names.
	 */
	...
	public static void makeColorSchemes() {
		...
		PImage image = new PImage(
				(new javax.swing.ImageIcon(new GCScheme().getClass().getResource("/data/default_gui_palette.png")))
						.getImage());
		// Now make the palette
		palettes = new Color[16][16];
		for (int p = 0; p < 16; p++)
