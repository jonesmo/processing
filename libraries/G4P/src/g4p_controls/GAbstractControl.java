...
public abstract class GAbstractControl implements PConstants, GConstants, GConstantsInternal {
	...
	/**
	 * This function can be used to help with text legibility when used with the Java Graphics2D 
	 * buffer. If required it should be called from makeBuffer method.
	 * 
	 * Currently this method is not called but is left here just in case. 
	 * 
	 * @param g2d Graphics2D buffer
	 * @param hint rendering hint
	 */
	private void setTextRenderingHints(Graphics2D g2d, int hint) {
		switch (hint) {
		case 0:
	...
	/**
	 * Return true if this class / control supports tool tips otherwise 
	 * false and output error message.
	 */
	protected boolean controlSupportsToolTips() {
	 ...
	 * @param offset	distance between control and tool tip centres
	 * @param angle		the angle (radians) made by the line joining the control 
	 * 					centre the tool tip centre
	 */
	public void setTip(GToolTip tt, float offset, float angle) {
		if (!controlSupportsToolTips()) return; 
		removeTip();
		tip = tt;
		tip.parent = this;
		addControlImpl(tip, 0, 0, 0);
		setTipPos(offset, angle);
	}
	 ...
	 * 					centre the tool tip centre
	 */
	public void setTip(String text, float offset, float angle) {
		if (!controlSupportsToolTips()) return; 
		removeTip();
	...
	/**
	 * Implementation method to set a tool tip based on polar position
	 * (offset and angle)
	 */
	public void setTipPos(float radius, float ang) {
		tip.setLocalColorScheme(localColorScheme);
		float dx = (float) Math.cos(ang) * radius + halfWidth - tip.halfWidth;
		float dy = (float) Math.sin(ang) * radius + halfHeight - tip.halfHeight;
		tip.moveTo(dx, dy, GControlMode.CORNER);
		//System.out.println(tag + "  " + dx + "  " + dy);
		hideTip();
	}
	 ...
	 * 
	 * @param tt	the custom tool tip
	 * @param ax	horizontal alignment with its control (LEFT, CENTER, RIGHT)
	 * @param ay	vertical position relative with its control (NORTH, SOUTH)
	...
	/**
	 * Implementation method to set a tool tip based on alignment 
	 * with control
	 */
	public void setTipPos(GAlign ax, GAlign ay, float gap) {
		tip.setLocalColorScheme(localColorScheme);
		float dx, dy;
		switch (ax) {
		case CENTER:
	...
	public void removeTip() {
		if (tip != null) {
			...
			tip.parent = null;		// clear parent reference in tip
	...
	/**
	 * Used internally to enforce minimum size constraints using
	 * current global control mode.<br>
	 * 
	 * This updates x, y, cx, cy, width and height so the original
	 * and resized control have the same centre position.
	 * 
	 * @param w
	 * @param h
	 */
	protected void resize(int w, int h) {
		resize(w, h, G4P.control_mode);
	}

	/**
	 * Used internally to enforce minimum size constraints using user
	 * defined control mode. <br>
	 * 
	 * This updates x, y, cx, cy, width and height so the original
	 * and resized control have the same centre position.
	 * 
	 ...
	 */
	protected void resize(int w, int h, GControlMode mode) {
		width = w;
		height = h;
		halfWidth = width / 2;
		halfHeight = height / 2;
	...
	/**
	 * Get the top-left x position of the control. <br>
	 * 
	 * Changed 3.4.10
	...
	/**
	 * Get the top-left y position of the control. <br>
	 * 
	 * Changed 3.4.10
	...
	/**
	 * Get the centre x position of the control centre. <br>
	 * 
	 * Changed 3.4.10
	...
	/**
	 * Get the centre y position of the control centre. <br>
	 * 
	 * Changed 3.4.10
	...
	public float getCY() {
		//		if (parent != null)
		//			return y + (parent.height + height) / 2.0f;
		//		else
		return cy;
	}
