...
/**
 * The provides an extremely configurable GUI knob controller. GKnob inherits
 * from GValueControl so you should read the documentation for that class as it
 * also applies to GKnob. <br>
 * <br>
 * 
 ...
 * User can specify mouse sensitivity for modes (b) and (c) Use can specify
 * easing to give smoother rotation
 * 
 * <b>Note</b>: Angles are measured clockwise starting in the positive x
 * direction i.e.
 * 
 * <pre>
 *         270
...
public class GKnob extends GValueControl {
	...
	/**
	 * Will create the a circular knob control that fits the rectangle define by the
	 * values passed as parameters. <br>
	 ...
	 * 
	 * <br>
	 * The radius of the inner griper radius is calculated from the bezel radius and
	...
	public GKnob(PApplet theApplet, float p0, float p1, float p2, float p3, float gripAmount) {
		...
		hotspots = new HotSpot[] { new HScircle(1, width / 2, height / 2, gripRadius) };
		z = Z_SLIPPY;
		...
		createEventHandler(G4P.sketchWindow, "handleKnobEvents", new Class<?>[] { GValueControl.class, GEvent.class },
	...
	/**
	 * The radius of the inner griper radius is calculated from the bezel radius and
	 * the parameter gripAmount using <br>
	 * 
	 * <pre>
	 * grip radius = bezel radiius * gripAmount
	 * </pre>
	...
	/**
	 * Decides when the knob will respond to the mouse buttons. If set to true it
	 * will only respond when over the arc made by the start and end angles. If
	...
	/**
	 * Convenience method to set both the show and the mouse over arc only
	 * properties for this knob
	 * 
	...
	public void mouseEvent(MouseEvent event) {
		...
		case MouseEvent.WHEEL:
			if (currSpot > -1 && z >= focusObjectZ()) {
				System.out.print(
						tag + "  " + parametricTarget + "  " + event.getCount() + "   " + wheelDelta + "     >>>  ");
				parametricTarget = calcParametricTarget(
						parametricTarget + event.getCount() * wheelDelta * G4P.wheelForSlider);
			...
			if (bezelWidth > 0) {
				...
				if (showTrack) {
					...
					buffer.arc(0, 0, 2 * (gripRadius + bezelWidth * 0.5f), 2 * (gripRadius + bezelWidth * 0.5f),
							PApplet.radians(startAng), PApplet.radians(anglePos));
				}
			}
	 ...
	 * GKnob.CTRL_ANGULAR or GKnob.CTRL_HORIZONTAL) orGKnob.CTRL_VERTICAL any other
	...
	/**
	 * This gets the sensitivity to be used in modes CTRL_HORIZONTAL and
	 * CTRL_VERTICAL
	 * 
	...
	public void setSensitivity(float sensitivity) {
		int sgn = sensitivity < 0 ? -1 : 1;
		float mag = Math.abs(sensitivity);
		this.sensitivity = sgn * ((mag < 0.1f) ? 0.1f : mag);
	}

	/**
	 * Calculates the 'angle' from the current mouse position based on the type of
	 * 'controller' set.
	 * 
