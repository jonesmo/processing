...
void setup() {
  surface.setTitle("Mandelbrot Explorer (Multiple windows)");
  createColours();
  createInstructions();
...
void processMouseEvent(PApplet appc, GWinData data, MouseEvent event) {
  ...
  case MouseEvent.RELEASE:
    ...
    if (deltaX >= 1 && deltaY >= 1) {
      ...
      G4P.showMessage(this, "Selection box size must be at least 1x1 pixel.", "Invalid Selection Box", G4P.WARN_MESSAGE);
    }
    break;
