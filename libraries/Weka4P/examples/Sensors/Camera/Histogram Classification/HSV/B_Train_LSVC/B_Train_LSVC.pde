...
String dataSetName = "HSVHistTrain"; 
String[] attrNames;
...
void setup() {
  ...
  video.start();
  wp.loadTrainARFF("HSVHistTrain.arff");                        //load a ARFF dataset
...
void draw() {
  background(255);
  updateChannels();
  Mat matH = hHist.getMat();
  Mat matS = sHist.getMat();
  Mat matV = vHist.getMat();
  for (int i = 0; i < binNum; i++) {
    rawData[0*binNum+i] = (float)matH.get(i, 0)[0];
    rawData[1*binNum+i] = (float)matS.get(i, 0)[0];
    rawData[2*binNum+i] = (float)matV.get(i, 0)[0];
  }
